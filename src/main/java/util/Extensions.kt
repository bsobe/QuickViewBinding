...
fun XmlTag.findLayoutTag(): PsiElement? {
    val dataTag = findFirstDataTag()
    val dataTagIndex = children.indexOfFirst { it == dataTag }
    return children.drop(dataTagIndex + 1).first { it is XmlTag }
}

fun XmlTag.addAttributes(newAttributes: List<XmlAttribute>) {
    val currentAttributes = attributes
    currentAttributes.forEach { setAttribute(it.name, null) }
    newAttributes.forEach { setAttribute(it.name, it.value) }
    currentAttributes.forEach { setAttribute(it.name, it.value) }
}